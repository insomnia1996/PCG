...
class DataLoader:
    ...
    def get_batch(self):
        ...
        max_cont_len+=len(gpt_context)
        for summary, text, field, pos, rpos, dec_field, dec_pos, dec_rpos, plan, plan_id, key_label, cont_text in data_subset:
            ...
            plan = plan + [0] * (max_plan_len - plan_len)
            plan_id = plan_id + [0] * (max_order_len - order_len)

            context = [self.empty] * (max_cont_len - cont_len - len(gpt_context)) + gpt_context + cont_text
            context_bart = cont_text + [self.empty] * (max_cont_len - cont_len - len(gpt_context))
            ...
            if max_text_len > self.man_text_len:
                ...
                context_bart = context_bart[:self.man_text_len]
            ...
            '''
            print("context:==============")
            for context in batch_data['context']:
                print(context)
            print("gpt context:==============")
            for context in batch_data['gpt_context']:
                print(context)
            '''
        return batch_data
